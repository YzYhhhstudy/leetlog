...
打通 LeetCode 和本地知识库：在题目页**敲下第一个键**自动开始计时，每次提交自动计数，
**AC 的瞬间**把通过代码、用时、提交次数写进 Obsidian 笔记。重做旧题自动累积在同一篇笔记里，
温故知新。你唯一要做的事：写感悟。
...
```
LeetCode 页面
  │  interceptor.js（拦截提交请求和判题结果，检测首次击键）
  ▼
content.js ──POST──▶ 本地桥接服务 127.0.0.1:8763（leetlog_server.py）
                          │  查题目元数据、计时、计数、组装 Markdown
                          ▼
                你的 Obsidian vault/LeetCode/0013-roman-to-integer.md
```

不抓取页面 DOM（LeetCode 改版就失效），而是拦截网络层：提交请求体里有你的代码，
判题接口里有 Accepted/运行时间/内存——这是最稳定的数据源。
**所有数据只在 `leetcode.com 页面 → 127.0.0.1 → 本地文件` 之间流动，无任何外部上传。**
...
Chrome / Edge / Arc：`chrome://extensions` → 打开右上角"开发者模式" → "加载已解压的扩展程序" → 选 `extension/` 文件夹。
...
## 第 1 次 · 2026-07-03 周五
⏱ 开始 10:34 → AC 10:49 · 用时 15 分钟 · 提交 2 次 / 通过 1 次
...
- 判题结果通过双通道获取：拦截到 submission_id 后主动轮询 `/check/` 接口，失效自动切换 GraphQL `submissionDetails` 兜底（不依赖页面 DOM，不受 UI 改版影响）
- 服务重启不丢状态（会话存在 `~/.config/leetlog/state.json`）
- 服务没开时做题？事件会丢（浏览器控制台有 `[LeetLog]` 提示）。可用姊妹项目 [lc-notes](../lc-notes) 的 `lc sync` 补记

## 已知边界

- 依赖 LeetCode 当前的提交接口（`/submit/` + `/submissions/detail/<id>/check/`）；接口大改需要更新 interceptor（比 DOM 稳定得多）
...
- [ ] 扩展侧离线队列：本地服务/插件不在线时把事件存 chrome.storage，恢复后补发，永不丢事件
